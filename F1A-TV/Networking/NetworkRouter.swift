...
//                print(String(data: data!,encoding: .utf8)!)
    ...
    func getNationLookup(nationUrl: String, completion: @escaping(Result<NationDto, APIError>) -> Void) {
        let request = RequestHelper.createRequestWithoutAuthentication(restService: nationUrl, method: "GET")
        ...
        let task = self.session.dataTask(with: request, completionHandler: {
            ...
            let requestResult: NationDto
            do {
                requestResult = try self.decoder.decode(NationDto.self, from: data!)
                completion(.success(requestResult))
            } catch {
                completion(.failure(.decodingError))
                return
    ...
    func getSeriesLookup(seriesUrl: String, completion: @escaping(Result<SeriesDto, APIError>) -> Void) {
        let request = RequestHelper.createRequestWithoutAuthentication(restService: seriesUrl, method: "GET")
        ...
        let task = self.session.dataTask(with: request, completionHandler: {
            ...
            let requestResult: SeriesDto
            do {
                requestResult = try self.decoder.decode(SeriesDto.self, from: data!)
                completion(.success(requestResult))
            } catch {
                completion(.failure(.decodingError))
                return
    ...
    func getTeamLookup(teamUrl: String, completion: @escaping(Result<TeamDto, APIError>) -> Void) {
        let request = RequestHelper.createRequestWithoutAuthentication(restService: teamUrl, method: "GET")
